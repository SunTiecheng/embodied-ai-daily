...
| `FEISHU_KEYWORD` | 飞书关键词，需与飞书机器人安全设置中的关键词一致（当前可用关键词为 `简报`） |

**注意：**
- 以上凭据仅通过环境变量读取，禁止写入仓库任何文件。
- `FEISHU_KEYWORD` 必须与飞书机器人「安全设置」里的自定义关键词完全一致；若配置不一致会返回 `19024 Key Words Not Found`。
- 消息标题仍会包含「日报」，以满足关键词过滤与可读性要求。
